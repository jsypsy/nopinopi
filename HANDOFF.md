...
**높이높이 — Phase 0 프로토타입 + 실기기 1차 반영 (BUILD 3). B1 실기기에서 나온 둘(최고기록 저장 회귀 버그·소리 추가)을 B2에 넣고, B2에서 나온 하나(소리 토글 표시 불일치)를 B3에서 뺐다. 디자인 개편 시안 3안은 사용자가 반려 — 카툰 유지. B3 테스트 배포 `01a0680a…` 문자 전송 완료, 판정 대기.**
...
코드       BUILD 3 — core(game·daily·tuning·rng) · render/renderer(하늘 고도 변화·잘린 조각·팝업·결과 카드) · main(카메라·이어하기·streak) · audio(WebAudio 합성 3종)
...
0. **B3 실기기 판정** (링크는 문자로 감). 먼저 이번 수정 둘을 확인:
   - **최고기록 회귀(D-004)가 잡혔는가** — 두 판 연속 낮은 층수를 내도 「오늘·역대」가 내려가지 않아야 한다. **미확인**(사용자가 B2에서 이 항목은 언급 안 함)
   - **소리(D-003)**: 탭 '톡'·딱 '띵'(콤보마다 반음 상승)·빗나감 '쿵'이 피드백으로 기능하는가, 볼륨 0.16~0.2가 시끄럽지 않은가(`src/audio.ts` gain). 타이틀에 알약이 없는지
   그다음 B1부터 남은 판정 항목:
...
1. **Phase 1 잔여** — 순서를 바꿨다. 사용자가 "계정별 랭킹은 어디서 보나"를 물었다(리더보드 버튼이 없어 답이 "아직 없다"였다):
   ① 결과 카드 [순위] 버튼(`platform.openLeaderboard`, 어댑터엔 이미 있음 — 콘솔 앱 정보 승인 전엔 LEADERBOARD_NOT_FOUND가 정상이라 실기기 확인은 승인 뒤) ·
   ② 시작 화면 데모 카드(날아날아 `drawStartDemo` 문법) · ③ [공유] · ④ 뒤로가기 확인 모달(플레이북 §3) ·
